...
def craw_parking():
    ...
    for i in range(31):
        ...
        num.append(data_word)
        data_list = data_list[end_data_index+5:]

        first_data_index = data_list.find("<td>") + 4
        end_data_index = data_list.find(" </td>")
        data_word = data_list[first_data_index:end_data_index]
        parking_place.append(data_word)
        data_list = data_list[end_data_index+5:]

        first_data_index = data_list.find("<td>") + 4
        end_data_index = data_list.find(" </td>")
        data_word = data_list[first_data_index:end_data_index]
        parking_max.append(data_word)
        data_list = data_list[end_data_index+5:]

        first_data_index = data_list.find("<td>") + 4
        end_data_index = data_list.find(" </td>")
        data_word = data_list[first_data_index:end_data_index]
        parking_now.append(data_word)
        data_list = data_list[end_data_index+5:]

        current_time = time.strftime("%Y-%m-%d %a %H", time.localtime(time.time()))
        now_time.append(current_time)
...
def save():
    ...
    f.close
    print("[+] save done! " + now_time[0])

schedule.every(60).minutes.do(save)
